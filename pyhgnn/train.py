...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25, print_freq=500):
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            ...
            with torch.set_grad_enabled(phase == 'train'):
                temp1 = time.time();
                outputs,t1,t2,tmm,tspmm = model(fts, G)
                t_fp += (time.time()-temp1);
    ...
    print('Forward time: {:.4f}s'.format(t_fp))
    print('Backward time: {:.4f}s'.format(t_bp))
    print('Layer1 time: {:.4f}s'.format(t_l1))
    print('Layer2 time: {:.4f}s'.format(t_l2))
    print('gemm time: {:.4f}s'.format(t_mm))
    print('spgemm time: {:.4f}s'.format(t_spmm))
